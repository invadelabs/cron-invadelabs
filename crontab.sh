...
0 6 * * * S=drew.invadelabs.com; D=/var/www/$S/sitemap; mv $D/sitemap-$S-NS_0-0.xml $D/sitemap-$S-NS_0-0.$(date '+\%Y\%m\%d\%H\%M\%S').xml; php /var/www/$S/maintenance/generateSitemap.php --compress no --fspath=/var/www/$S/sitemap/ --identifier=$S --urlpath=https://$S/ --server=https://$S > /dev/null
